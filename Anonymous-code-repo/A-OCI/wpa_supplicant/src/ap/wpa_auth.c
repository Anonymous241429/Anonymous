...
//Anonymous
    // u8 rsn_data[50] = kde->rsn_ie;
//	 size_t rsn_data_len = kde->rsn_ie_len;
     wpa_hexdump(MSG_DEBUG, "Anonymous Msg 2 received **-->",
		    kde->rsn_ie, kde->rsn_ie_len);     
   // u8 * signA = hmac_sha1(kde->mic, 20, buf1, sizeof(buf1), nonce);//Anonymous
   // os_memcpy(signC,kde->rsn_ie+22,23)
	//if(signC != SignA)
	//	{
...
SM_STATE(WPA_PTK, AUTHENTICATION2)
{
	...
	wpa_hexdump(MSG_DEBUG, "WPA: Assign Anonymous ANonce", sm->ANonce,
		    WPA_NONCE_LEN-2);
...
#endif /* CONFIG_DPP2 */
//struct hostapd_iface *iface = sm->ctx->ctx;
...
SM_STATE(WPA_PTK, PTKCALCNEGOTIATING)
{
	...
	wpa_hexdump(MSG_DEBUG, "Anonymous Msg 2 received **-->",
		    key_data, key_data_length);  //Anonymous
...
//Anonymous
     ...
     wpa_hexdump(MSG_DEBUG, "Anonymous Msg 2 received **-->",
		    kde.rsn_ie, 45);     
   //hmac_sha1(&PTK, 20, buf3, sizeof(buf3), sm->SNonce);//Anonymous
	wpa_hexdump(MSG_DEBUG, "Anonymous Msg sign received **-->",
		    &PTK, 16);
			//wpa_hexdump(MSG_DEBUG, "Anonymous Msg buf3 received **-->",
		    //buf3, 16);
			wpa_hexdump(MSG_DEBUG, "Anonymous Msg ANonce received **-->",
		    sm->SNonce, 16);
   // os_memcpy(signC,kde->rsn_ie+22,23)
	//if(signC != SignA)
	//	{
   //         wpa_sta_disconnect(wpa_auth, sm->addr,
	...
	forced_memzero(&PTK, sizeof(PTK));
	sm->PTK_valid = TRUE;
	wpa_hexdump(MSG_DEBUG, "Anonymous Msg sign at last received **-->",
		    &sm->PTK.kck, 16);
}
...
SM_STATE(WPA_PTK, PTKINITNEGOTIATING)
{
	...
	wpa_auth_logger(sm->wpa_auth, sm->addr, LOGGER_DEBUG,
			"sending 3/4 msg of 4-Way Handshake - Anonymous Updated this message");
	if (sm->wpa == WPA_VERSION_WPA2) {
