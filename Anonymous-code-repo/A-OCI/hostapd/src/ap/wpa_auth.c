...
SM_STATE(WPA_PTK, PTKCALCNEGOTIATING)
{
	...
	key_data = mic + mic_len + 2;
	key_data_length = WPA_GET_BE16(mic + mic_len);
	// (key_data_length > sm->last_rx_eapol_key_len - sizeof(*hdr) -
	...
	/* Anonymous updated this part of code......*/
	
	wpa_hexdump(MSG_DEBUG, "Anonymous Msg2 key information received **-->", key_data, key_data_length);
	wpa_hexdump(MSG_DEBUG, "Anonymous Msg2 RSN information received **-->", kde.rsn_ie, 45);
	wpa_hexdump(MSG_DEBUG, "Anonymous Msg2 WPA information received **-->", kde.wpa_ie, 45);
