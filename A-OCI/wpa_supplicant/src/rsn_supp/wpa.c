			       ...
			       struct wpa_ptk *ptk)
{
	...
	if (wpa_key_mgmt_ft(sm->key_mgmt)) {
		int res;
       //wpa_ie_len = wpa_ie_len+2; //Anonymous
		/*
...
#endif /* CONFIG_IEEE80211R */
    wpa_ie_len = wpa_ie_len+23; //Anonymous
	...
	printf("*-*-%02x", wpa_ie[17]);
	printf("*-*-%02x", wpa_ie[18]);
	printf("*-*-%02x", wpa_ie[19]);
	printf("*-*-%02x", wpa_ie[20]);
	u8 * wpa_ie_new = wpa_ie;
	//wpa_ie_new[20] = 0xdd;
	//wpa_ie_new[21] = 0xee;
	//wpa_ie_new[22] = 0xff;// Anonymous
	//wpa_ie_new[23] = 0xcc;//Anonymous
	//wpa_ie_new[24] = 0xdd;//Anonymous
	//wpa_ie_new[25] = 0xee;//Anonymous
	//wpa_ie_new[43] = 0xcc;//Anonymous
	//wpa_ie_new[44] = 0xdd;//Anonymous
	//u8 *buf1 = wpa_ie;
	

	u8 *buf2[30];
	//wpa_hexdump(MSG_DEBUG, "WPA: nonce data",
	//		    nonce, 32);
	hmac_sha1(&sm->tptk.kck, 16, nonce, 32, buf2);//Anonymous
	u8 *wpa_ie_sign = buf2;//Anonymous
    os_memcpy(wpa_ie_new + 22, wpa_ie_sign, 20);//Anonymous
	//wpa_hexdump(MSG_DEBUG, "WPA: nonce data",
	//		    nonce, 20);
	//wpa_hexdump(MSG_DEBUG, "WPA: key data",
    //			    &sm->tptk.kck, 16);
	//			wpa_hexdump(MSG_DEBUG, "WPA: sign data",
	//		    wpa_ie_sign, 5);
	//			wpa_hexdump(MSG_DEBUG, "WPA: buf data",
	//		    buf2, 5);
    //wpa_ie_new[26] = 0xbb;//Anonymous
	wpa_ie_new[42] = 0xcc;//Anonymous
	wpa_ie_new[43] = 0xdd;//Anonymous
	wpa_ie_new[44] = 0xee;//Anonymous
	key_mic = (u8 *) (reply + 1);
	//wpa_ie_len = wpa_ie_len+2; //Anonymous

	
	wpa_hexdump(MSG_DEBUG, "WPA: new data",
			    wpa_ie_new, 45);
	WPA_PUT_BE16(key_mic + mic_len, wpa_ie_len); /* Key Data Length */
	os_memcpy(key_mic + mic_len + 2, wpa_ie_new, wpa_ie_len); /* Key Data */
	os_free(rsn_ie_buf);

	os_memcpy(reply->key_nonce, nonce, WPA_NONCE_LEN);

	wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: Sending EAPOL-Key 2/4");
	buf1[0] = ((uint32_t)key_info >> 8) & 0xFFu;
    buf1[1] = ((uint32_t)key_info >> 0) & 0xFFu;
	//printf("%x\n", &key_mic);
	wpa_hexdump(MSG_DEBUG, "WPA: Derived Key MIC Anonymous",
			    key_mic, mic_len);
	printf("------->");
	//printf("0x%X "%"PRIX8 "\n", buf1[0], buf1[1]);
	printf("0x%X\n", buf1[0]);
					   ...
					   u8 *key_data, size_t *key_data_len)
{
	...
	wpa_hexdump_key(MSG_DEBUG, "WPA: decrypted EAPOL-Key key data",
			key_data, *key_data_len);
	wpa_hexdump(MSG_DEBUG, "WPA: decrypted EAPOL-Key key data Anonymous",
			key_data, *key_data_len);
	return 0;
}
