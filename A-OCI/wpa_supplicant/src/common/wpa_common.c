...
#define MAX_Z_LEN 66 /* with NIST P-521 */
   ...
   FILE* ptr;
    ...
    ptr = fopen("/etc/hostapd/hostapd.confg", "r");
   ...
   char buffer[256];
    ...
    while (fgets(buffer, sizeof(buffer), ptr))
        if (1 == sscanf(buffer, " channel = %127s", value))
            printf("Value = %s\n", value);
    int value1 = atoi(value);
	nonce1_new[31] = value1;
	wpa_hexdump(MSG_DEBUG, "WPA: Anonymous Nonce1", nonce1, WPA_NONCE_LEN);
