	...
	struct wpa_supplicant *wpa_s, const u8 *bssid)
{
    ...
    //struct wpa_ssid *bssid1 = wpa_s->bssid;
    struct wpa_driver_auth_params params;
	if (ssid->ssid_len > 0)
		{bss = wpa_bss_get(wpa_s, bssid, ssid->ssid, ssid->ssid_len);
		printf("***hello****");
		//printf("%s", wpa_s->bssid);
		printf("***hi****");}
		i++;
		printf("%d",i);
		params.bssid=bss->bssid;
		wpa_msg(wpa_s, MSG_INFO, "printing BSSID " MACSTR
					  ...
					  union wpa_event_data *data)
{
	...
	if (data->assoc_info.resp_ies) {
		wpa_hexdump(MSG_DEBUG, "resp_ies_Anonymous", data->assoc_info.resp_ies,
